...
public class ConnectorTest {
    ...
    public void importLog() {
        ...
        try {
            String sourceJson = FileUtils.readFileToString(new File(url.getFile() + "/sample/test/source-kafka.json"), "utf-8");
            String sourceAccountJson = FileUtils.readFileToString(new File(url.getFile() + "/sample/test/source-kafka-account.json"), "utf-8");
            String sinkJson = FileUtils.readFileToString(new File(url.getFile() + "/sample/test/sink-clickhouse.json"), "utf-8");

            vo.setName("test-log");
            vo.setSourceName("maxwell-kafka");
